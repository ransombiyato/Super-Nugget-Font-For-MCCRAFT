...
public class DimensionTrickRecipe extends TrickRecipe {
	...
	public DimensionTrickRecipe(@Nullable PieceCraftingTrick piece, Ingredient input, ItemStack output, ItemStack cad, Identifier dimensionKey) {
	...
	public @NotNull RecipeType<? extends net.minecraft.world.item.crafting.Recipe<SingleRecipeInput>> getType() {
		return (RecipeType<? extends net.minecraft.world.item.crafting.Recipe<SingleRecipeInput>>) (RecipeType<?>) ModCraftingRecipes.DIMENSION_TRICK_RECIPE_TYPE.get();
	}

	@Override
	public @NotNull RecipeSerializer<? extends net.minecraft.world.item.crafting.Recipe<SingleRecipeInput>> getSerializer() {
	...
	public static class Serializer implements RecipeSerializer<DimensionTrickRecipe> {
		...
		private static final MapCodec<DimensionTrickRecipe> CODEC = RecordCodecBuilder.mapCodec(
				instance -> instance.group(
						...
						Identifier.CODEC.fieldOf("dimensionKey").forGetter(d -> d.dimensionKey.identifier())
				)
		...
		private static DimensionTrickRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			Identifier dimensionId = Identifier.STREAM_CODEC.decode(buf);
		...
		private static void toNetwork(RegistryFriendlyByteBuf buf, DimensionTrickRecipe recipe) {
			...
			Identifier.STREAM_CODEC.encode(buf, recipe.dimensionKey.identifier());
		}
