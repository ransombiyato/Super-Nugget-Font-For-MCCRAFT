...
public class TrickRecipe implements ITrickRecipe {
	...
	public @NotNull RecipeType<? extends net.minecraft.world.item.crafting.Recipe<SingleRecipeInput>> getType() {
		return (RecipeType<? extends net.minecraft.world.item.crafting.Recipe<SingleRecipeInput>>) (RecipeType<?>) ModCraftingRecipes.TRICK_RECIPE_TYPE.get();
	}
	...
	public static class Serializer implements RecipeSerializer<TrickRecipe> {
		...
		private static final MapCodec<TrickRecipe> CODEC = RecordCodecBuilder.mapCodec(
				instance -> instance.group(
						...
						Identifier.CODEC.optionalFieldOf("piece").forGetter(d -> d.piece != null ? Optional.of(d.piece.registryKey) : Optional.empty())
				)
		...
		private static TrickRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			if(ByteBufCodecs.BOOL.decode(buf)) {
				trick = (PieceCraftingTrick) SpellPiece.create(Identifier.STREAM_CODEC.decode(buf));
			}
		...
		private static void toNetwork(RegistryFriendlyByteBuf buf, TrickRecipe recipe) {
			...
			if(recipe.piece != null) {
				...
				Identifier.STREAM_CODEC.encode(buf, recipe.piece.registryKey);
			} else {
