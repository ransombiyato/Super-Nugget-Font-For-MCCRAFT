...
public class ItemCAD extends Item implements ICAD {
				...
				if(context.cspell.metadata.evaluateAgainst(cad)) {
					...
					if(PsiEventBus.post(event).isCanceled()) {
						...
						if(cancelMessage != null && !cancelMessage.isEmpty()) {
							player.displayClientMessage(Component.translatable(cancelMessage).setStyle(Style.EMPTY.withColor(ChatFormatting.RED)), false);
						}
					...
					player.displayClientMessage(Component.translatable("psimisc.weak_cad").setStyle(Style.EMPTY.withColor(ChatFormatting.RED)), false);
				}
			}
		}
	...
	@NotNull
	@Override
	public InteractionResult use(@NotNull Level worldIn, Player playerIn, @NotNull InteractionHand hand) {
		...
		if(playerCad != itemStackIn) {
			if(!worldIn.isClientSide()) {
				playerIn.displayClientMessage(Component.translatable("psimisc.multiple_cads").setStyle(Style.EMPTY.withColor(ChatFormatting.RED)), false);
			}
		...
		for(ItemEntity item : items) {
			...
			Optional<RecipeHolder<ITrickRecipe>> recipe =
					ModCraftingRecipes.RECIPE_TYPES.getEntries().stream()
							...
							.map(ro -> ((net.minecraft.server.level.ServerLevel) world).getServer().getRecipeManager().getRecipeFor((ModCraftingRecipes.PsiTrickRecipeType<ITrickRecipe>) ro.get(), inv, world))
