...
public class PieceSelectorNearbySmeltables extends PieceSelectorNearby {
	...
	public static ItemStack simulateSmelt(Level world, ItemStack input) {
		...
		return server.getServer().getRecipeManager().getRecipeFor(RecipeType.SMELTING, new SingleRecipeInput(input), world)
				.map(foo -> foo.value().assemble(new SingleRecipeInput(input), RegistryAccess.EMPTY))
	...
	public boolean accept(Entity e) {
		if(e instanceof ItemEntity eitem) {
			return !simulateSmelt(e.level(), eitem.getItem()).isEmpty();
		}
