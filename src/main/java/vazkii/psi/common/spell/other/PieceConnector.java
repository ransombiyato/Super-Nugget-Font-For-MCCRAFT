...
public class PieceConnector extends SpellPiece implements IRedirector {
	...
	public static final Identifier LINES_TEXTURE = Identifier.parse(LibResources.SPELL_CONNECTOR_LINES);
	...
	private void drawSide(PoseStack pPoseStack, MultiBufferSource buffers, int light, SpellParam.Side side) {
		if(side.isEnabled()) {
			Material material = new Material(net.minecraft.client.renderer.texture.TextureAtlas.LOCATION_BLOCKS, LINES_TEXTURE);
			VertexConsumer buffer = material.buffer(net.minecraft.client.Minecraft.getInstance().getAtlasManager(), buffers, ignored -> SpellPiece.getLayer());
