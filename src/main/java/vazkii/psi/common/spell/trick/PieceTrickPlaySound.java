...
public class PieceTrickPlaySound extends PieceTrick {
	...
	public Object execute(SpellContext context) throws SpellRuntimeException {
		...
		var instruments = context.focalPoint.level().registryAccess().lookupOrThrow(net.minecraft.core.registries.Registries.INSTRUMENT).stream().toList();
		if(instruments.isEmpty()) {
			return null;
		}
		int instrumentId = Mth.clamp((int) instrumentVal, 0, instruments.size() - 1);

		float f = (float) Math.pow(2, (pitchVal - 12) / 12.0);
		context.focalPoint.level().playSound(null, pos, instruments.get(instrumentId).soundEvent().value(), SoundSource.RECORDS, (float) volVal, f);
		return null;
	}
}
