...
public class PieceTrickSmeltBlock extends PieceTrick {
	...
	public Object execute(SpellContext context) throws SpellRuntimeException {
		...
		if(!context.focalPoint.level().mayInteract(context.caster, pos)) {
			return null;
		}
		...
		if(!result.isEmpty()) {
			...
			if(block1 != Blocks.AIR) {
				context.focalPoint.level().setBlockAndUpdate(pos, block1.defaultBlockState());
				context.focalPoint.level().levelEvent(2001, pos, Block.getId(block1.defaultBlockState()));
			}
		}
