...
public class PieceTrickSmeltBlockSequence extends PieceTrick {
	...
	public Object execute(SpellContext context) throws SpellRuntimeException {
		...
		for(BlockPos blockPos : MathHelper.getBlocksAlongRay(positionVal.toVec3D(), positionVal.copy().add(targetNorm.copy().multiply(maxBlocksInt)).toVec3D(), maxBlocksInt)) {
			...
			if(!context.focalPoint.level().mayInteract(context.caster, blockPos)) {
				return null;
			}

			BlockState state = context.focalPoint.level().getBlockState(blockPos);
			...
			ItemStack result = PieceSelectorNearbySmeltables.simulateSmelt(context.focalPoint.level(), stack);
			if(!result.isEmpty()) {
				Item item = result.getItem();
				Block block1 = Block.byItem(item);
				if(block1 != Blocks.AIR) {
					context.focalPoint.level().setBlockAndUpdate(blockPos, block1.defaultBlockState());
					context.focalPoint.level().levelEvent(2001, blockPos, Block.getId(block1.defaultBlockState()));
				}
			}
