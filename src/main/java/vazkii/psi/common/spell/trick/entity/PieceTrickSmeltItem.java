...
public class PieceTrickSmeltItem extends PieceTrick {
	...
	public Object execute(SpellContext context) throws SpellRuntimeException {
		...
		if(targetVal instanceof ItemEntity eitem && targetVal.isAlive()) {
			...
			ItemStack result = PieceSelectorNearbySmeltables.simulateSmelt(eitem.level(), stack);
			...
			if(!result.isEmpty()) {
				...
				ItemEntity item = new ItemEntity(context.focalPoint.level(), eitem.getX(), eitem.getY(), eitem.getZ(), result.copy());
				context.focalPoint.level().addFreshEntity(item);
			}
		} else {
