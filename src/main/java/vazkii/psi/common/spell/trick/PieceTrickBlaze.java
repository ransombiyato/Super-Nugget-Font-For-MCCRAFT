...
public class PieceTrickBlaze extends PieceTrick {
	...
	public Object execute(SpellContext context) throws SpellRuntimeException {
		...
		BlockEvent.EntityPlaceEvent placeEvent = new BlockEvent.EntityPlaceEvent(BlockSnapshot.create(context.focalPoint.level().dimension(), context.focalPoint.level(), pos), context.focalPoint.level().getBlockState(pos.relative(Direction.UP)), context.caster);
		PsiEventBus.post(placeEvent);
		if(placeEvent.isCanceled()) {
			return null;
		}
		if(state.isAir() || state.canBeReplaced()) {
			context.focalPoint.level().setBlockAndUpdate(pos, Blocks.FIRE.defaultBlockState());
		} else {
			pos = pos.above();
			state = context.focalPoint.level().getBlockState(pos);
			if(state.isAir() || state.canBeReplaced()) {
				context.focalPoint.level().setBlockAndUpdate(pos, Blocks.FIRE.defaultBlockState());
			}
		}
