...
public class PieceTrickSmite extends PieceTrick {
	...
	public Object execute(SpellContext context) throws SpellRuntimeException {
		...
		BlockEvent.EntityPlaceEvent placeEvent = new BlockEvent.EntityPlaceEvent(BlockSnapshot.create(context.focalPoint.level().dimension(), context.focalPoint.level(), positionVal.toBlockPos()), context.focalPoint.level().getBlockState(positionVal.toBlockPos().relative(Direction.UP)), context.caster);
		PsiEventBus.post(placeEvent);
		if(placeEvent.isCanceled()) {
			return null;
		}

		if(context.focalPoint.level() instanceof ServerLevel) {
			...
			context.focalPoint.level().addFreshEntity(lightning);
		}
