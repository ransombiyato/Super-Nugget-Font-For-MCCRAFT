...
public class PieceTrickPlaceInSequence extends PieceTrick {
	...
	public Object execute(SpellContext context) throws SpellRuntimeException {
		...
		if(directionVal != null) {
			direction = Direction.getNearest((int) Math.signum(directionVal.x), (int) Math.signum(directionVal.y), (int) Math.signum(directionVal.z), Direction.NORTH);
			horizontalFacing = Direction.getNearest((int) Math.signum(directionVal.x), 0, (int) Math.signum(directionVal.z), Direction.NORTH);
		}
		...
		for(BlockPos blockPos : MathHelper.getBlocksAlongRay(positionVal.toVec3D(), positionVal.copy().add(targetNorm.copy().multiply(maxBlocksInt)).toVec3D(), maxBlocksInt)) {
			...
			PieceTrickPlaceBlock.placeBlock(context.caster, context.focalPoint.level(), blockPos, context.getTargetSlot(), false, direction, horizontalFacing);
		}
