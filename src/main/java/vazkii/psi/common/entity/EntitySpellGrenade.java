...
public class EntitySpellGrenade extends EntitySpellProjectile {
	...
	public void doExplosion() {
		...
		double m = 0.1;
		for(int j = 0; j < 40; j++) {
			double d0 = level().random.nextGaussian() * m;
			double d1 = level().random.nextGaussian() * m;
			double d2 = level().random.nextGaussian() * m;
			...
			level().addParticle(ParticleTypes.EXPLOSION, x, y, z, d0, d1, d2);
		}
	}
	...
	protected void onHit(@NotNull HitResult ray) {
		if(ray instanceof EntityHitResult && ((EntityHitResult) ray).getEntity() instanceof LivingEntity) {
			setAttackTarget(((EntityHitResult) ray).getEntity());
		}
		if(!level().isClientSide() && !sound && explodes()) {
