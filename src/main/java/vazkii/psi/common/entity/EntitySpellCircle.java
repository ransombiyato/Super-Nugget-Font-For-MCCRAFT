...
public class EntitySpellCircle extends Entity implements ISpellImmune {
	...
	private static final EntityDataAccessor<Optional<net.minecraft.world.entity.EntityReference<LivingEntity>>> CASTER_UUID = SynchedEntityData.defineId(EntitySpellCircle.class, EntityDataSerializers.OPTIONAL_LIVING_ENTITY_REFERENCE);
	...
	public EntitySpellCircle setInfo(Player player, ItemStack colorizer, ItemStack bullet) {
		...
		entityData.set(CASTER_UUID, Optional.of(net.minecraft.world.entity.EntityReference.of(player)));
	...
	protected void addAdditionalSaveData(net.minecraft.world.level.storage.ValueOutput output) {
		...
		entityData.get(CASTER_UUID).ifPresent(reference -> net.minecraft.world.entity.EntityReference.store(reference, output, TAG_CASTER));
		output.putInt(TAG_TIME_ALIVE, getTimeAlive());
		output.putInt(TAG_TIMES_CAST, entityData.get(TIMES_CAST));
		output.putFloat(TAG_LOOK_X, entityData.get(LOOK_X));
		output.putFloat(TAG_LOOK_Y, entityData.get(LOOK_Y));
		output.putFloat(TAG_LOOK_Z, entityData.get(LOOK_Z));
		output.putFloat(TAG_DIRECTION_X, entityData.get(DIRECTION_X));
		output.putFloat(TAG_DIRECTION_Y, entityData.get(DIRECTION_Y));
		output.putFloat(TAG_DIRECTION_Z, entityData.get(DIRECTION_Z));
		output.putFloat(TAG_SCALE, entityData.get(SCALE));
		output.putInt(TAG_LIFETIME, entityData.get(LIFETIME));
	}
	...
	protected void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput input) {
		...
		entityData.set(CASTER_UUID, Optional.ofNullable(net.minecraft.world.entity.EntityReference.read(input, TAG_CASTER)));
		setTimeAlive(input.getIntOr(TAG_TIME_ALIVE, 0));
		entityData.set(TIMES_CAST, input.getIntOr(TAG_TIMES_CAST, 0));
		entityData.set(LOOK_X, input.getFloatOr(TAG_LOOK_X, 0.0F));
		entityData.set(LOOK_Y, input.getFloatOr(TAG_LOOK_Y, 0.0F));
		entityData.set(LOOK_Z, input.getFloatOr(TAG_LOOK_Z, 0.0F));
		entityData.set(DIRECTION_X, input.getFloatOr(TAG_DIRECTION_X, 0.0F));
		entityData.set(DIRECTION_Y, input.getFloatOr(TAG_DIRECTION_Y, 1.0F));
		entityData.set(DIRECTION_Z, input.getFloatOr(TAG_DIRECTION_Z, 0.0F));
		entityData.set(SCALE, input.getFloatOr(TAG_SCALE, 1.0F));
		entityData.set(LIFETIME, input.getIntOr(TAG_LIFETIME, (CAST_TIMES + 2) * CAST_DELAY));
	}
	...
	public LivingEntity getCaster() {
		return entityData.get(CASTER_UUID).map(reference -> net.minecraft.world.entity.EntityReference.getLivingEntity(reference, level())).orElse(null);
	}
