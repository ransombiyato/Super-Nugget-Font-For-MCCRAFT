...
public final class SpellGrid {
	...
	public void readFromNBT(CompoundTag cmp) {
		...
		ListTag list = cmp.getListOrEmpty(TAG_SPELL_LIST);
		int len = list.size();
		for(int i = 0; i < len; i++) {
			CompoundTag lcmp = list.getCompoundOrEmpty(i);
			int posX, posY;

			if(lcmp.contains(TAG_SPELL_POS_X_LEGACY)) {
				posX = lcmp.getIntOr(TAG_SPELL_POS_X_LEGACY, 0);
				posY = lcmp.getIntOr(TAG_SPELL_POS_Y_LEGACY, 0);
			} else {
				posX = lcmp.getIntOr(TAG_SPELL_POS_X, 0);
				posY = lcmp.getIntOr(TAG_SPELL_POS_Y, 0);
			}

			CompoundTag data;
			if(lcmp.contains(TAG_SPELL_DATA_LEGACY)) {
				data = lcmp.getCompoundOrEmpty(TAG_SPELL_DATA_LEGACY);
			} else {
				data = lcmp.getCompoundOrEmpty(TAG_SPELL_DATA);
			}
