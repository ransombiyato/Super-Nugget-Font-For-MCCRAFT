...
public final class Spell {
	...
	public static Spell createFromNBT(CompoundTag cmp) {
		if(cmp == null || !cmp.getBooleanOr(TAG_VALID, false)) {
			return null;
		}
	...
	public void readFromNBT(CompoundTag cmp) {
		name = cmp.getStringOr(TAG_SPELL_NAME, "");

		if(cmp.contains(TAG_UUID_MOST)) {
			long uuidMost = cmp.getLongOr(TAG_UUID_MOST, 0L);
			long uuidLeast = cmp.getLongOr(TAG_UUID_LEAST, 0L);
