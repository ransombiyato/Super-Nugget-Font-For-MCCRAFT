...
public abstract class SpellPiece {
	...
	public static RenderType getLayer() {
		if(layer == null) {
			layer = RenderTypes.entityTranslucent(TextureAtlas.LOCATION_BLOCKS);
		}
		return layer;
	}
	...
	public static SpellPiece createFromNBT(Spell spell, CompoundTag cmp) {
		...
		if(cmp.contains(TAG_KEY_LEGACY)) {
			key = cmp.getStringOr(TAG_KEY_LEGACY, "");
		} else {
			key = cmp.getStringOr(TAG_KEY, "");
		}
		...
		if(PsiAPI.SPELL_PIECE_REGISTRY.containsKey(rl)) {
			...
			Set<String> pieceNamespaces = PsiAPI.SPELL_PIECE_REGISTRY.keySet().stream().map(Identifier::getNamespace).collect(Collectors.toSet());
			for(String namespace : pieceNamespaces) {
				rl = Identifier.fromNamespaceAndPath(namespace, key);
				if(PsiAPI.SPELL_PIECE_REGISTRY.containsKey(rl)) {
					exists = true;
					break;
				}
		...
		if(exists) {
			Class<? extends SpellPiece> clazz = PsiAPI.SPELL_PIECE_REGISTRY.getValue(rl);
	...
	public static SpellPiece create(Identifier location) {
	...
	public void drawBackground(PoseStack pPoseStack, MultiBufferSource buffers, int light) {
		Material material = ClientPsiAPI.SPELL_PIECE_MATERIAL_REGISTRY.getValue(registryKey);
		if(material == null) {
			return;
		}

		VertexConsumer buffer = material.buffer(Minecraft.getInstance().getAtlasManager(), buffers, ignored -> getLayer());
	...
	public void readFromNBT(CompoundTag cmp) {
		CompoundTag paramCmp = cmp.getCompoundOrEmpty(TAG_PARAMS);
		for(String s : params.keySet()) {
			SpellParam<?> param = params.get(s);

			String key = s;
			if(paramCmp.contains(key)) {
				paramSides.put(param, SpellParam.Side.fromInt(paramCmp.getIntOr(key, 0)));
			} else {
		...
		comment = cmp.getStringOr(TAG_COMMENT, "");
	}
